defmodule AcWeb.ProductControllerTest do
  use AcWeb.ConnCase

  alias Ac.Item

  @create_attrs %{cta: "some cta", display_name: "some display_name", draft: true, featured_image: "some featured_image", name: "some name", price: 120.5}
  ...
  def fixture(:product) do
    {:ok, product} = Item.create_product(@create_attrs)
    product
  end

  describe "index" do
    test "lists all products", %{conn: conn} do
      conn = get conn, product_path(conn, :homepage)
  ...
  describe "update product" do
    ...
    test "redirects when data is valid", %{conn: conn, product: product} do
      ...
      assert html_response(conn, 200) =~ "some updated cta"
    end
  ...
  describe "delete product" do
    ...
    test "deletes chosen product", %{conn: conn, product: product} do
      ...
      assert redirected_to(conn) == product_path(conn, :homepage)
